    ...
    <header className={`fixed w-full bg-transparent z-50 mnhdr h-auto transition-all ${scrolling ? 'blur-background' : ''} ${mobileMenuOpen ? 'h-24 blur-background' : ''}`}>
      <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
        ...
        <div className="flex space-x-4">
          ...
          <Link href="https://github.com/Nahidul-Islam-Siam/" target='_blank' className="hidden md:flex">
            <Image src="/github-mark-white.svg" alt="" className='GitHub' height={20} width={30}/>
          </Link>
          <Link href="https://www.linkedin.com/in/nahidul-islam-siam/" target='_blank' className="hidden md:flex">
            <FaLinkedin size={36}/>
          </Link>
        </div>
      </nav>
      <div className={`md:hidden ${mobileMenuOpen ? 'flex' : 'hidden'} justify-between mt-4 mb-6 ml-5`}>
        ...
        <div className="flex items-center mt-20 mr-4">
          ...
          <Link href="https://www.linkedin.com/in/nahidul-islam-siam/" target='_blank' className="ml-4">
            <FaLinkedin size={36}/>
          </Link>
        </div>
      </div>
